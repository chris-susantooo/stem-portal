...
class GameAddScene extends Phaser.Scene {
  constructor () {
    super('GameAddScene')
    this.scoreText = null
    this.score = 0
  ...
  create () {
    ...
    this.input.keyboard.on(Phaser.Input.Keyboard.Events.ANY_KEY_DOWN, (event) => {
      if (event.code) { // Digit1, KeyQ, Enter, Backspace, Numpad1
...
function createQuestion (game) {
  ...
  let symbol = game.add.text(450, 250, '+', { fontSize: '56px', fontFamily: 'Arial', color: '#00ff00' }).setStroke('#000000', 4)
...
export default new GameAddScene()
